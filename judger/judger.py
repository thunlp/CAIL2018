...
class Judger:
    ...
    def format_result(self, result):
        ...
        if not ("accusation" in result.keys()):
            result["accusation"] = []
        else:
            res = []
            for x in result["accusation"]:
                try:
                    ...
                    pass
            result["accusation"] = res

        if not ("imprisonment" in result.keys()):
            result["imprisonment"] = -3
        else:
            if not (type(result["imprisonment"]) is int):
                result["imprisonment"] = -3

        if not ("articles" in result.keys()):
            result["articles"] = []
        else:
            res = []
